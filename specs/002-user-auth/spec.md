...
- 當使用者的 Email 已經被註冊時會怎樣?
- 如何處理 Email 驗證連結過期的情況?
- 如果使用者上傳的頭像不是圖片格式會怎樣?
- 當使用者快速連續點擊「重新發送驗證 Email」按鈕時如何防止濫用?
...
- **FR-009**: 系統必須支援使用者填寫荒野夥伴資訊,要求輸入:荒野編號、所屬分會、自然名,不進行格式或真實性驗證
...
- **FR-012**: 系統必須允許使用者編輯個人檔案:顯示名稱、頭像
...
- **FR-018**: 系統必須允許使用者編輯自己提交的待審核地點
- **FR-019**: 系統必須在使用者登出時清除前端的驗證狀態
- **FR-020**: 系統必須根據顯示情境自動選擇適當的頭像尺寸(列表顯示使用 small,個人檔案使用 medium,全螢幕檢視使用 large)
...
- **SC-001**: 註冊流程從開始填寫到帳號建立成功,整個過程在 10 秒內完成
- **SC-002**: Email 驗證信在使用者註冊後 1 分鐘內送達
- **SC-003**: 登入流程(輸入帳密到跳轉地圖頁)在 3 秒內完成
- **SC-004**: 荒野夥伴資訊填寫後,Custom Claims 更新在 2 秒內完成
