...
3. **假設** 使用者拒絕 GPS 權限, **當** 權限被拒絕, **則** 顯示友善提示訊息並提供手動地址輸入選項；若使用者未輸入地址，則顯示台灣全島視圖（縮放至可見所有地點）
4. **假設** GPS 取得失敗(例如室內或訊號不良), **當** 超時或失敗, **則** 顯示錯誤訊息並提供手動地址輸入選項；若使用者未輸入地址，則顯示台灣全島視圖
...
- 當使用者位於 GPS 訊號不良的區域(例如:室內、地下室)時會怎樣? → ✅ T023 處理，顯示友善提示並提供手動地址輸入或台灣全島視圖
- 如何處理地圖上沒有任何地點標記的情況? → ✅ T053 處理，顯示「目前區域尚無綠色生活地點」提示
- 當使用者快速連續點擊多個標記時,詳情面板如何回應? → ✅ T054 處理，使用 debounce 防止連續點擊，僅顯示最後點擊的地點
- 如果選擇的標籤沒有任何地點時該如何顯示? → ✅ T055 處理，顯示「沒有符合篩選條件的地點」提示
- 當地點照片載入失敗時如何處理? → ✅ T033 處理，顯示預設佔位圖片
- 地圖縮放到極限時(放大或縮小)如何處理? → ⚙️ Google Maps 原生處理，已設定合理的 minZoom/maxZoom 範圍
...
- **FR-004**: 系統必須在地圖上一次載入並顯示所有已核准狀態(`status: 'approved'`)的綠色生活地點（初期預估少於 500 個地點，無需分頁；未來地點數量增加時再評估實作 Marker Clustering）
...
- **FR-012**: 系統必須支援多種方式關閉詳情面板：行動裝置支援向下滑動手勢及關閉按鈕(X)；桌面版支援點擊詳情面板外部區域或關閉按鈕
- **FR-013**: 系統必須在登錄者為荒野夥伴時,以「團名-自然名」格式顯示
- **FR-014**: 系統必須僅對已登入使用者顯示「回報此地點資訊有誤」按鈕
- **FR-015**: 系統必須在照片載入失敗時顯示預設佔位圖片

### 關鍵實體 *(包含因為功能涉及資料)*

- **Location (地點)**: 代表一個綠色生活地點，包含名稱（name）、地址（address）、經緯度（coordinates: GeoPoint）、描述（description）、照片 URL 陣列（photoUrls）、標籤 ID 陣列（tagIds）、狀態（status）、提交者資訊（submitterInfo）、時間戳記（createdAt, updatedAt）
